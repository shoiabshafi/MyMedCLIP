...
class TGVA(nn.Module):
    ...
    x' = x + Up(ReLU(h_down))
    """
    def __init__(self, c_in, bottleneck=768, text_dim=768, use_additive_gating=False, use_identity_gating=False):
        super(TGVA, self).__init__()
        self.use_additive_gating = use_additive_gating
        self.use_identity_gating = False # Default to False, can be set later or via init if modified
    ...
    def forward(self, x, prompt_emb):
        ...
        # Identity Gating Ablation: Override gates with 1s (Visual-only conditioning)
        if self.use_identity_gating:
            g_det = torch.ones_like(g_det)
            g_seg = torch.ones_like(g_seg)
...
class DPAS_CLIP(nn.Module):
    ...
    def __init__(self, args, clip_model):
        ...
        self.use_identity_gating = getattr(args, 'use_identity_gating', False)

        self.tgva_adapters = nn.ModuleList([
            TGVA(c_in=1024, bottleneck=768, text_dim=768, 
                 use_additive_gating=self.use_additive_gating,
                 use_identity_gating=self.use_identity_gating) 
